...
from kitty.constants import is_windows
from kitty.fast_data_types import truncate_point_for_length, wcswidth
from kitty.typing_compat import BossType, KeyEventType, TypedDict
from kitty.utils import ScreenSize

from ..tui.handler import Handler, result_handler
from ..tui.loop import Loop, MouseEvent
from ..tui.operations import MouseTracking, set_window_title, styled
...
text and :italic:`letter` is a single letter
...
class Password(Handler):
    # A single line of text input. Used for the line, password and file types.
    # The historic readline-based line editor needed a controlling tty and the
    # readline module, neither of which exist on Windows, so all three types now
    # share this loop-driven editor.
    ...
    def initialize(self) -> None:
        if self.cli_opts.title:
            self.write(set_window_title(self.cli_opts.title))
    ...
    def __init__(self, cli_opts: AskCLIOptions) -> None:
        ...
        if self.message and self.cli_opts.hidden_text_placeholder:
            ...
            if self.hidden_text_start_pos > -1:
                # Hidden text arrives on stdin, which kitty does not deliver to a
                # kitten on Windows, and reading the console there would block.
                self.hidden_text = '' if is_windows else sys.stdin.read().rstrip()
...
def main(args: list[str]) -> Response:
    ...
    if len(prompt) > 1 and prompt[0] == prompt[-1] and prompt[0] in '\'"':
        prompt = prompt[1:-1]
    is_password = cli_opts.type == 'password'
    # line and file share the plain editor; a full file browser is future work.
    loop = Loop()
    phandler = Password(cli_opts, prompt, is_password=is_password, initial_text='' if is_password else (cli_opts.default or ''))
    loop.loop(phandler)
